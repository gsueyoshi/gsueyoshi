<!--
**gsueyoshi/gsueyoshi** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->

<h1 align="center">Genki Sueyoshi</h1>
<p align="center"><strong>末吉 元気</strong></p>
<p align="center">
Webアプリや業務自動化ツールを中心に、実用的なものを少しずつ作っています。<br>
AIも開発に取り入れながら、アイデアを素早く形にして改善していく進め方が好きです。
</p>

<p align="center">
  <a href="https://github.com/gsueyoshi">
    <img src="https://img.shields.io/badge/GitHub-gsueyoshi-0D1117?style=for-the-badge&logo=github&logoColor=white&labelColor=1F6FEB" alt="GitHub">
  </a>
</p>

---

## プロフィール

| 項目 | 内容 |
| --- | --- |
| 名前 | 末吉 元気（Genki Sueyoshi） |
| 取り組み領域 | Web開発 / 業務自動化 / AI活用開発 |
| 得意なこと | 要件整理 / 実装しやすい仕様づくり / 継続的な改善 |
| 開発スタイル | 小さく作って試し、改善を重ねながら育てていく |

## 取り組んでいること

普段は、Webアプリケーションの実装や、日々の作業を少し楽にする自動化ツールづくりに取り組んでいます。  
「こういう仕組みがあると便利そう」という段階のアイデアを、まずは動く形にしてみて、そこから少しずつ使いやすく整えていく進め方が中心です。

技術としては、JavaScript / TypeScript / React を使ったフロントエンド寄りの開発に加えて、Python や Google Apps Script を使った自動化、AWS Lambda を活用したサーバーレスな処理などにも触れています。  
最近は、AIをコーディングや試作の補助に取り入れながら、開発速度と品質のバランスを探ることにも関心があります。

## 開発で大事にしていること

開発では、最初から大きく作り込むよりも、まず小さく作って早めに試すことを大切にしています。  
実際に動かしながら改善点を見つけて、あとから手を入れやすい形に整えていくほうが、自分には合っていると感じています。

また、コードそのものだけでなく、READMEや補足説明もできるだけ分かりやすく書くように意識しています。  
自分が後から見返したときにも扱いやすく、他の人が読んでも流れを追いやすいリポジトリを目指しています。

## GitHubについて

このGitHubでは、学習用の記録だけでなく、実際に動かせるものや、試作の過程が分かるものも残していきたいと考えています。  
完成したものを置くだけでなく、試しながら改善していく流れも含めて、自分の開発の積み重ねが見える場所にしていくつもりです。

そのため、実装サンプル、業務改善のための小さなツール、AI活用の検証コードなど、実用性や再利用しやすさを意識した内容を少しずつ公開しています。

## 技術スタック

<p>
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=000000" alt="JavaScript">
  <img src="https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=ffffff" alt="TypeScript">
  <img src="https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=0A0A0A" alt="React">
  <img src="https://img.shields.io/badge/Node.js-5FA04E?style=flat-square&logo=node.js&logoColor=ffffff" alt="Node.js">
  <img src="https://img.shields.io/badge/GAS-4285F4?style=flat-square&logo=googleappsscript&logoColor=ffffff" alt="Google Apps Script">
  <img src="https://img.shields.io/badge/AWS-232F3E?style=flat-square&logo=amazonaws&logoColor=white" alt="AWS">
  <img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=ffffff" alt="Python">
  <img src="https://img.shields.io/badge/SQL-336791?style=flat-square&logo=postgresql&logoColor=ffffff" alt="SQL">
</p>

